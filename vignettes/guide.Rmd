...
author: "[Phil Stubbings](http://parasec.net)"
date: "`r Sys.Date()`"
output: 
...
__bid.vwap25:500bps__ The VWAP for 20 25bps percentiles below the best bid.

   __best.ask.price__ The best ask price.

     __best.ask.vol__ Amount of volume available at the best ask.

 __ask.vol25:500bps__ The amount od volume available for 20 25bps percentiles 
                      above the best ask.

__ask.vwap25:500bps__ The VWAP for 20 25bps percentiles above the best ask.
--------------------------------------------------------------------------------
...
resiliance (the rate at which market makers fill a void after a market impact).
...
the event driven "leapfrog" phenomenon discussed preivously.
...
order book which would effect the VWAP (Volume Weighted Aeverge Price) of a 
market order. 
...
activity of individual market particicpants. By filtering the display of
cancellation events within a volume range, it is possible to isolate what are
most likely inidividual order placement strategies. The following graph shows an
...
thier market order. In addition, the _hits_ column shows the number of _hit_

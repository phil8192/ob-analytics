...
remove.duplicates <- function(events) {
  ...
  duplicate.deletes <- deletes[deletes$id %in% deletes[which(duplicated(deletes$id)), ]$id, ]
  duplicate.event.ids <- duplicate.deletes[duplicated(duplicate.deletes$id), ]$event.id
  print(paste("removed", length(duplicate.event.ids), "duplicate order cancellations: ", 
...
load.event.data <- function(file) {
  ...
  print(paste("loading data from", file))

  events <- read.csv(file, header=T, sep=",")
  events$timestamp <- as.POSIXct(events$timestamp/1000, origin="1970-01-01", tz="UTC")
  events$exchange.timestamp <- as.POSIXct(events$exchange.timestamp, origin="1970-01-01", tz="UTC")
  ...
  # order quote data by id, then by volume (decreasing), then finally, by order of action: created,changed,deleted.
  # then finally, in the case of multiple changes and no volume change (price update) order by our timestamp. (this is for exchanges that allow in-place quote updates)
  events <- events[order(events[, "id"], -events[, "volume"], events[, "action"], events[, "timestamp"]), ]

  events <- cbind(event.id=1:nrow(events), events)
  events <- remove.duplicates(events)

  fill.deltas <- unlist(tapply(events$volume, events$id, vector.diff), use.names=F)
  # for pacman orders, do not log volume for price update events.
  fill.deltas <- ifelse(unlist(tapply(events$price, events$id, vector.diff), use.names=F) == 0, fill.deltas, 0)
  events <- cbind(events, fill=abs(fill.deltas))

  ### fix timestamps: most of the time the event stream is out of order.
  ### the events have been ordered by id, volume, then action. now re-assign the timestamps to match this order for each order id.
  print("realigning event timestamps...")
...
# distance of bid/ask limit order from best bid/ask (immediately before order added/deleted) in bps.
...
order.aggressiveness <- function(events, depth.summary) {
  event.diff.bps <- function(events, direction) {
    orders <- events[events$direction == ifelse(direction == 1, "bid", "ask") & events$action != "changed" & (events$type == "flashed-limit" | events$type == "resting-limit"), ]
    orders <- orders[order(orders$timestamp), ]
    stopifnot(all(orders$timestamp %in% depth.summary$timestamp))
    best <- depth.summary[match(orders$timestamp, depth.summary$timestamp), ifelse(direction == 1, "best.bid.price", "best.ask.price")]
  ...
  events[match(bid.diff.bps$event.id, events$event.id), ]$aggressiveness.bps <- bid.diff.bps$diff.bps

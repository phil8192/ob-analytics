...
active.order.state <- function(quotes, tp=as.POSIXlt(Sys.time(), tz="UTC"), min.bid=0, max.ask=Inf) {
  # current active orders (all active orders + their entire lifecycle)
  created.before<-quotes[quotes$action=="created" & quotes$timestamp <= tp, "id"]
  deleted.before<-quotes[quotes$action=="deleted" & quotes$timestamp <= tp, "id"]
  ...
  # for partial fills (order changed) <= now, keep the most recent change (least remaining volume),
...
bids <- function(active.orders) {
  bids <- active.orders[active.orders$direction=="bid" & active.orders$type != "market", ]
  # order by price, then each price level is fifo, ordered by order id (could also be timestamp)
  bids <- bids[order(-bids$price, bids$id), ]
  first.price <- head(bids, 1)$price
  cbind(bids, pct=(first.price-bids$price)/first.price, liquidity=cumsum(bids$volume))
}

asks <- function(active.orders) {
  asks <- active.orders[active.orders$direction=="ask" & active.orders$type != "market", ]
  asks <- asks[order(asks$price, asks$id), ]
  first.price <- head(asks, 1)$price
  cbind(asks, pct=(asks$price-first.price)/first.price, liquidity=cumsum(asks$volume))
}

# get a nice order book
current.order.book <- function(active.orders, max.price.levels=NULL, pct.range=0, ts="") {
  asks <- asks(active.orders)[, c("id", "timestamp", "exchange.timestamp", "price", "volume", "liquidity", "pct")]
  asks <- asks[rev(1:nrow(asks)), ] # reverse the asks (ascending price)
  bids <- bids(active.orders)[, c("id", "timestamp", "exchange.timestamp", "price", "volume", "liquidity", "pct")]
  if(pct.range > 0) {
